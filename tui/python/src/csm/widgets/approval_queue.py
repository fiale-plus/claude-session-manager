...
Simplified: safety classification now comes from the daemon's pending_tools
list, so no local classifier or JSONL parsing is needed.
"""
...
from csm.models import Session, ToolSafety
...
class ApprovalQueue(Vertical):
    ...
    def update_queue(self, sessions: list[Session]) -> None:
        ...
        Pending tools and their safety classification come directly from
        the daemon via each session's pending_tools field.
        """
        rows: list[_QueueRow] = []
        for session in sessions:
            for pt in session.pending_tools:
                auto_approved = (
                    session.autopilot
                    and pt.safety in (ToolSafety.SAFE, ToolSafety.UNKNOWN)
                )
                rows.append(_QueueRow(
                    session=session,
                    tool_name=pt.tool_name,
                    tool_input=pt.tool_input,
                    safety=pt.safety,
    ...
    def _refresh_display(self) -> None:
        ...
        for i, row in enumerate(self._rows):
            ...
            name = (row.session.project_name or row.session.session_id or "?")[:8].ljust(8)
